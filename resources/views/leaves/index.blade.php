        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-2 lg:gap-3">
            ...
            <form action="{{ route('leaves.index') }}" method="GET" class="flex items-center gap-2">
                <div class="relative">
                    ...
                    <input type="search" name="search" value="{{ request('search') }}" placeholder="Search" class="pl-9 lg:pl-10 w-40 sm:w-52 rounded-lg border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 text-sm py-2">
                </div>
            </form>
            <form action="{{ route('leaves.index') }}" method="GET" class="flex items-center gap-2">
                @php
